...
for LOOP in range(loop):
    #Wczytanie datasetu
    load_df = pd.read_csv("elecequip.csv", header =0, index_col= 0)
    ...
    train, test = supervised[:146],supervised[146:]
    ...
    #Dopasowanie modelu
    lstm_model = fit_lstm(scaled_train,1,2000,32)
    print(lstm_model)
    ...
    # report performance
    rmse = sqrt(mean_squared_error(raw_values[-49:], predictions))
    mape_val = mape(raw_values[-49:],predictions)
    print('Test RMSE ' + str(LOOP) + ': %.3f' % rmse)
    print(mape(raw_values[-49:],predictions))
    rmse_list.append(rmse)
    mape_list.append(mape_val)
    
    # line plot of observed vs predicted
    plt.plot(raw_values[-49:])
